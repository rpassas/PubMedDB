...
```{sql connection=mydb}
INSERT INTO starschema.ArticlesSummary(num_articles, time_id, journal_title, author_id)
SELECT count(starschema.ArticleFactTable.article_id) as num_articles, 
...
```
#Grouping by qarter, seems to be more articles published in the winter. 
```{sql connection=mydb}
select quarter, count(num_articles) from starschema.ArticlesSummary 
