  ...
  for(i in 1:nrow(Article.df)){
    if(Article.df$article_title[i] == artTitle){
      for(j in 1:nrow(Issue.df)){
        if(Issue.df$journal_title == title && Issue.df$issue == issue 
           && Issue.df$volume == volume ){
...
```{sql connection=mydb}
select * from Article
JOIN Journal using(journal_title)
```
...
```{sql connection=mydb}
select * from JournalIssue
JOIN Article using(journal_title)
order by journal_title
```
