...
parseHistory <- function (aHistoryNode, i)
{
  ...
  for (m in 1:n)
  {
    ...
    HistoryStatus.df[historystatusrow,3] <- pub_status
    HistoryStatus.df[historystatusrow,4] <- hisDate
...
parseHistory2 <- function (aHistoryNode, i)
{
  newHistory.df <- History.df <- data.frame (history_id = integer(),
                          article_id = integer(),
                          pub_status = character(),
...
for (i in 1:numArticles) #should be numArticles not 3
{
  ...
  tempIssue <-JournalIssue.df
  JournalIssue.df <- rbind(tempIssue,newIssue.df)
