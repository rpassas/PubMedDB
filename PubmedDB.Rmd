...
parseJournals <- function (anArticle)
{
  ...
  firstChild <- tempPubDate[[1]]
  print(firstChild)
  Year <- xmlElementsByTagName(test, "Year")
  MedlineDate <- xmlElementsByTagName(test, "MedlineDate")
  
  print(Year[1])
  print(length(Year))
  print(length(MedlineDate))
...
```{r Q1}
xpathexpression <-"//PubmedArticle/MedlineCitation/Article/Journal/JournalIssue/PubDate"
x <- xpathSApply(xmlDOM,xpathexpression)
test <- x[[1]]
te <- xmlElementsByTagName(test, "Me")
print(length(te))
if(length(te)!=0) {
  print("it works")
}
...
#In medline citation pull all but the hisotry
...
=======
dbDisconnect(mydb)
>>>>>>> 3eb570d26615da8a7ba4211f06543a3bc0118653
```
