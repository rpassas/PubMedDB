...
CREATE TABLE IF NOT EXISTS starschema.JournalDimension 
  AS SELECT Journal.journal_title,
            JournalIssue.issue,
            JournalIssue.volume,
...
```{sql connection=mydb}
CREATE TABLE starschema.ArticleFactTable (
  article_id INT NOT NULL PRIMARY KEY,
  article_title TEXT NOT NULL,
...
```{sql connection=mydb}
CREATE TABLE starschema.TimeDimension (
...
```{sql connection=mydb}
INSERT INTO starschema.TimeDimension (year, month, quarter)
  SELECT CAST(SUBSTRING(pub_date, 1,4) AS UNSIGNED) as year, 
...
```{sql connection=mydb number of articles per period}
CREATE TABLE IF NOT EXISTS starschema.QuarterlySummary(
...
```{sql connection=mydb}
INSERT INTO starschema.QuarterlySummary(journal_title, author_first, author_last,
article_count, year, quarter)
SELECT Journal.journal_title, Author.first_name AS author_first, 
  Author.last_name AS author_last,
  count(Article.ArticleTitle) AS article_count, 
  CAST(SUBSTRING(JournalIssue.pub_date,1,4) AS UNSIGNED) AS year,
  0 AS quarter
  FROM dbpracticum2.Article 
  JOIN dbpracticum2.AuthorJoin USING(article.id);
  JOIN dbpracticum2.Author USING(author.id);
  --JOIN dbpracticum2.Journal USING(journal_title);
  JOIN dbpracticum2.JournalIssue USING(journal_title);
```

```{sql connection=mydb}
UPDATE starschema.QuarterlySummary SET quarter = 
  CASE
    WHEN quarter <= 3 THEN 1
    WHEN quarter <= 6 THEN 2
    WHEN quarter <= 9 THEN 3
    ELSE 4
  END
WHERE quarter = 0;
```
